 ...
 *
 * git check-ref-format が禁止する文字 (制御文字, スペース, ~ ^ : ? * [ \)
 * は containsForbiddenChars または git 側で弾かれる。シェルメタ文字
 * (; & | ` $ ( ) < > " ' { }) は containsForbiddenChars で defense-in-depth
 * として別途禁止している。
 *
 * 追加の禁止条件は isValidBase 内で個別に判定する:
 * - `..` を含まない
 * - `@{` を含まない (parseRevision で先行判定)
 * - `//` を含まない
 * - 末尾 `/`
 * - 末尾 `.lock`
 * - 制御文字
