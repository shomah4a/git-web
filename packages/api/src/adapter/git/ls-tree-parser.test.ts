...
describe('parseLsTreeZ', () => {
  ...
  it('複数エントリをパースできる', () => {
    ...
    expect(result[2]).toEqual({ name: 'README.md', path: 'README.md', type: 'blob', status: null })
  })
  ...
  it('basePath が指定されている場合はパスに前置される', () => {
    ...
    expect(parseLsTreeZ(input, 'src')).toEqual([
      { name: 'index.ts', path: 'src/index.ts', type: 'blob', status: null },
    ])
  })
  ...
  it('ネストしたパスの name は最後のセグメントになる', () => {
    ...
    expect(parseLsTreeZ(input, 'src')).toEqual([
      { name: 'DiffView.vue', path: 'src/components/DiffView.vue', type: 'blob', status: null },
    ])
  })
  ...
  it('日本語ファイル名をパースできる', () => {
    ...
    expect(parseLsTreeZ(input, '')).toEqual([
      { name: '設計書.md', path: 'ドキュメント/設計書.md', type: 'blob', status: null },
    ])
  })
})
