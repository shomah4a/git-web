...
describe('parseLogOutput', () => {
  ...
  it('numstat 付きの単一コミットをパースできる', () => {
    const input =
      '\0abc1234\x01Alice\x01alice@example.com\x012026-04-21T10:00:00+09:00\x01add files\x01\x01' +
      '10\t2\tsrc/foo.ts\n3\t0\tsrc/bar.ts\n'
  ...
  it('バイナリファイルの numstat は insertions/deletions が 0 として扱われる', () => {
    const input =
      '\0abc1234\x01Alice\x01alice@example.com\x012026-04-21T10:00:00+09:00\x01add image\x01\x01' +
      '-\t-\timage.png\n5\t1\tREADME.md\n'
  ...
  it('複数コミットをパースできる', () => {
    const input =
      '\0aaa1111\x01Alice\x01alice@example.com\x012026-04-21T10:00:00+09:00\x01first\x01body1\x01' +
      '1\t0\ta.ts\n' +
      '\0bbb2222\x01Bob\x01bob@example.com\x012026-04-21T11:00:00+09:00\x01second\x01\x01' +
      '2\t3\tb.ts\n'
  ...
  it('改行を含むコミットメッセージ body をパースできる', () => {
    const body = 'line1\nline2\nline3'
    const input =
      '\0abc1234\x01Alice\x01alice@example.com\x012026-04-21T10:00:00+09:00\x01subject\x01' +
      body +
      '\x01' +
      '1\t0\ta.ts\n'
  ...
  it('マージコミットで numstat が空の場合は統計が全て 0 になる', () => {
    const input =
      '\0abc1234\x01Alice\x01alice@example.com\x012026-04-21T10:00:00+09:00\x01Merge branch main\x01\x01'
