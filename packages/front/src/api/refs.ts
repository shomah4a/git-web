...
/**
 * refs 一覧 API 呼び出しのクライアント層 (ADR 0018 / ADR 0019)。
 *
 * 設計方針:
 * - レスポンスは unknown で受けて型ガードで narrow する (`as` 禁止、ADR 0010)
 * - クエリは URLSearchParams 経由で組み立て、`q` / `limit` を安全にエンコードする
 * - 4xx/5xx は throw する。呼び出し側 (RevisionCombobox) で catch し、候補欄の
 *   フォールバック表示に倒す
 */

/**
 * GET /api/refs?q=<q>&limit=<limit> を呼んでブランチ/タグ一覧を取得する。
 *
 * @param q     部分一致フィルタ文字列 (空文字列可)
 * @param limit 返却件数の上限 (1〜500)
 */
export async function fetchRefs(q: string, limit: number): Promise<RefListDto> {
