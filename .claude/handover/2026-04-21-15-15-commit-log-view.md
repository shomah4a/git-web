...
3. アダプタ層: git log パーサー (NUL/SOH 区切り、%P で親ハッシュ取得)、`CliGitClient.log` メソッド
4. 共通 DTO: `CommitDto` / `CommitsResponseDto` (parentCount 含む)
...
8. マージコミット判定: parentCount >= 2 で merge バッジ表示
