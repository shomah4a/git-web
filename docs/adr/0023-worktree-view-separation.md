...
ディレクトリ配下の変更ファイルの status を集約して表示する。statusMap のキーをディレクトリプレフィックスで走査して判定するため、追加の git コマンドは不要。

- 配下に変更なし → null (表示なし)
- 配下の変更タイプが全て同一 → そのタイプをそのまま表示 (例: untracked のみ → `?`)
- 複数タイプが混在 → `'modified'` (M) で代表
