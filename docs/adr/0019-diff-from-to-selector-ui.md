...
本コンポーネントのテキストフィールドは **検索ボックス** として位置付ける。入力中は親の `modelValue` を更新せず、候補の絞り込みと候補リスト表示にのみ使う。確定 (commit) はユーザーの明示操作 (候補クリック / Enter) または暗黙操作 (blur) のみで行う。
...
- 候補リスト構成順:
  ...
  3. `refs.branches`
  4. `refs.tags`
- 入力文字列変更時: 内部 `inputText` のみ更新し、debounce 200ms で `fetchRefs(inputText, 50)` を再取得する。`update:modelValue` / `submit` の emit は行わない
- 並行発火時は `lastIssuedGen` を使った世代カウンタで後発優先とする (AbortController は導入しない)
- **明示確定** (候補クリック / Enter): `emit('update:modelValue', value)` + `emit('submit', value)` → `isOpen=false`
  - DiffView 側はこの `submit` を受けて `runDiffLoad` を自動発火する (自動適用)
- **Enter で自由入力確定**: highlight 無しで Enter を押したときは `inputText` をそのまま `modelValue` に確定し、同時に `submit` も emit する
- **暗黙確定 (blur)**: フォーカス外れ時、`inputText` が `modelValue` と異なれば `update:modelValue` のみ emit する (`submit` は emit しない)
  - 理由: フォーカス外しは「入力中→別の操作へ移動」の自然な流れであり、この契機で diff を自動再ロードするとユーザー意図に反する。親 state の取り漏らしだけ防ぐ
  - blur は 150ms 遅延でクローズする。これは候補 `mousedown.prevent` とのレースで候補クリックを先に拾うため
- Esc キー: 候補リストを閉じる (inputText は戻さない)
...
テキストフィールド右側に SVG inline の虫眼鏡アイコン (feather-icons `search` 同型) を CSS `background-image` で配置する。追加依存を入れずに済ませるため、ライブラリ / フォント / 画像アセットは使わない。

#### unmount 時

- 保留中の debounce setTimeout と blur 遅延 setTimeout の両方を `clearTimeout` で破棄する
...
`<button :disabled="loadingList" @click="onApply">適用</button>`。
loadingList 中は disabled にし、連打で意図不明な race を招かないようにする。

適用ボタンは自動適用に加えて残す。用途:

- blur commit 経由で入力した値の適用 (blur は submit を emit しないため)
- 同じ値のままリロードしたい経路 (worktree 編集後の再読み込み等)

#### 4.6 自動適用

`RevisionCombobox` の `submit` を DiffView が listen し、`onRevSubmit` で `runDiffLoad(currentRange())` を自動発火する。v-model の emit は `submit` より先に走るため、`currentRange()` は最新の `fromRev`/`toRev` を拾う。

`loadingList` 中でも自動適用は走らせる。既存の generation カウンタが race を吸収するため、前発の処理は自動的に破棄される。適用ボタンの disabled はユーザー明示操作に対する二重押下防止のみに適用する。
