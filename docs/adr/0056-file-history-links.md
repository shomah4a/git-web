...
- ファイル一覧 (RevisionTreeView / WorktreeView) の Last commit message / date 列からファイル history へリンクしたい
...
新規ビューやエンドポイントは追加しない。既存の `/commits?rev=<rev>&path=<file>` ルートに導線を増やすだけにする。

ADR 0046 の `path` 仕様は単一ファイルに限定していないが、本 ADR ではユーザー要望に従い **blob (ファイル) のみリンク化** する。ディレクトリ単位の集約履歴は将来の検討対象とする (§Future Work)。

The Zen of Python "There should be one obvious way to do it" に従い、ファイル history への入り口は `/commits?path=` の単一経路にする。
...
- リンク化対象は `entry.type === 'blob'` かつ `lastCommit !== null` の行のみ
- `entry.type === 'tree'` の行はテキスト表示のまま (本 ADR スコープ外)
- リンク先: `/commits?rev=<rev>&path=<entry.path>`

行クリックの blob 遷移と二重発火を避けるため、リンク要素には `@click.stop` を付ける。
...
### D. ディレクトリ単位の集約 history を即時サポート

`git log -- <dir>` で技術的には可能だが、ユーザー要望は「ファイル単位」であり、ディレクトリのリンク化は意味論が異なる (集約結果になる) ため、別 ADR で扱う。
