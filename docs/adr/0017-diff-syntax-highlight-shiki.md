...
- v2 (本版): 防衛評価を全件反映。Shiki API (4.0.2) を実査した結果で設計を具体化
...
| kotlin                   | あり                                           |
| ruby                     | あり                                           |
| bash                     | `shellscript` のエイリアスとして受け付けられる |
| markdown                 | あり                                           |
| json / yaml / html / css | あり                                           |
| toml                     | あり                                           |

全て silent fallback 不要で色が付く見込み。`bash` については Shiki が alias として解決するが、実装時に `codeToTokens` に `bash` を渡してエラーが出ないことを単体確認する。
...
- `content.length > 512 * 1024` (512KB 相当) または 行数 > 5000 の場合、該当サイドはプレーン
...
本番は `createShikiHighlighter()`、テストは `createNoOpHighlighter()`。no-op は全行を `[{ content: line, color: null }]` として即時 resolve する。
...
- window 全体の `scrollY` も退避・復元する (ファイル単位の内側 scroll ではないが、max-content の height 変動で絶対位置が動くケースの保険)
...
- scroll sync の再 setup は不要。ただし tokenMap 差し替え前後で scrollLeft を保存/復元
- 512KB 閾値により大容量 minified JS / lockfile の UI フリーズを回避
