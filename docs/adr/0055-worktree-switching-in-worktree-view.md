...
WorktreeView の本来の目的 (作業ディレクトリの現在の状態を見る) からして、`.gitignore` で除外されているが作業ツリーに実体としては存在する `.claude/tmp/`, `node_modules/`, `packages/*/dist/` などを **可視化する**。これらは `--exclude-standard` で従来除外されていたが、本 ADR で取り扱いを変更する。

実装方針:

- `WorktreeEntryStatus(Dto)` に **`ignored`** を追加 (ADR 0023 の `added/modified/deleted/untracked/null` を拡張)
- WorktreeLister で `git ls-files -z --others --ignored --exclude-standard --directory` を追加実行
- `--directory` により ignored なディレクトリは末尾 `/` 付きでまとめて 1 行に出る (中身を再帰列挙しない)
- 既存の tracked/untracked エントリと name 衝突するものは追加しない (tracked 側の status を優先)
- フロント側で status='ignored' を `I` 表示 (色は faint + opacity 0.7 で薄め)

### 11. UI 実装上の注意 (実装で確定した事項)

#### 11.1 selector の確定経路

`WorktreeCombobox` を `<select>` で実装し、`@submit` のみ親 (WorktreeView) で listen する。`@update:model-value` も emit はするが listen しない。両方 listen すると `@update:model-value` 経由で先に親 state が更新され、続く `@submit` 内の「同値なら no-op」ガードが誤動作してしまうため。

#### 11.2 WorktreeCombobox の `v-if` ガード

`worktrees` 一覧が取得済み (空でない) のときのみ selector を描画する。空の `<select>` を Teleport 内で描画すると vitest 環境で Vue の patch が `emitsOptions: null` で落ちることが観測されたため、defensive にガードする。

## 関連 ADR

- ADR 0011: API レイヤリング
- ADR 0019: from/to のリビジョン指定コンボボックス (UI 作法の参照元)
- ADR 0023: WorktreeView の分離 (status の集合を ADR 0055 で拡張)
